...
function send_notification( &$model, &$rec ) {
  
  // get local references to the database and request objects
  // this is inane and should be replaced by global $db, $req
  global $db;
  global $request;
  $req =& $request;
  ...
  // update the reference to the table we are notifying about
        ...
        if ($p) {
          ...
          if (is_email($i->email_value) && !(in_array($i->email_value, $sent_to))) {
            ...
            if (isset($i->token) && strlen($i->token) > 0)
              $addr = $req->url_for(array('resource'=>$notify_table,'id'=>$notify_id,'ident'=>$i->token));
            else
              $addr = $req->url_for(array('resource'=>$notify_table,'id'=>$notify_id));
            ...
            $subject = $profile->given_name.$actionmessage.classify($req->resource);
            ...
            if (!empty($i->token_url)) {
              
              // discover postNotice URL via Yadis/xrds-simple
              
              if (!empty( $i->post_notice )) {
                
                $_SESSION['access_token'] = $i->token_url;
                $_SESSION['post_notice'] = $i->post_notice;
                $_SESSION['id'] = $notify_id;
                $_SESSION['resource'] = $notify_table;
                
                redirect_to( $request->url_for('oauth_continue') );
